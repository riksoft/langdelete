...
class admin_plugin_langdelete extends DokuWiki_Admin_Plugin
{
   ...
   function handle()
   {
      ...
      if ($submit) {
         ...
         if (array_diff($lang_keep, $shortlang) || array_diff($shortlang, $lang_keep)) {
            $d->discrepancy = true;
         }
      } else {
         // Keep every language on first run
   ...
   function print_shortlist(&$d)
   {
      ...
      echo '<input type="hidden" name="shortlist[' . self::DEFAULT_LANG . ']"' . ' form="langdelete__form" />';
      ...
      foreach ($d->u_langs as $l) {
         ...
         echo '<input type="checkbox" id="shortlang-' . $l . '"' . ' name="shortlist[' . $l . ']"' . ' form="langdelete__form"' . ($checked ? ' checked' : '') . ($l == self::DEFAULT_LANG ? ' disabled' : '') . ' />';
   ...
   private function html_print_langs($langs, $keep = null)
   {
      ...
      $print_lang_li = function ($langs) use ($keep) {
         ...
         foreach ($langs as $val) {
            ...
            echo '<li val="' . $val . '"' . ($enabled ? ' class="enabled"' : '') . '>';
            if ($enabled) {
               echo $val;
            } else {
               echo '<del>' . $val . '</del>';
   ...
   private function list_languages()
   {
      ...
      $dir_subfolders = function ($dir) {
         ...
         $sub = array_filter($sub, function ($e) use ($dir) {
            return is_dir("$dir/$e") && !in_array($e, ['.', '..']);
         });
         return $sub;
      };
      ...
      return [
         ...
         "templates" => array_combine($templates, array_map($list_langs, array_prefix($templates, DOKU_INC . "lib/tpl/"))),
         "plugins" => array_combine($plugins, array_map($list_langs, array_prefix($plugins, DOKU_PLUGIN))),
      ];
   }
